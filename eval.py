...
def fast_eval(seq='00',model_file="./model/attention/00.ckpt",desc_file='./data/desc_kitti/00.npy',pair_file='./data/pairs_kitti/neg_100/00.txt'):
    net=RINet_attention()
    net.load(model_file)
    net.to(device=device)
    net.eval()
    ...
    np.savetxt("result/"+seq+'.txt',np.concatenate([scores.reshape(-1,1),gt.reshape(-1,1)],axis=1))
...
def recall(seq='00',model_file="./model/attention/00.ckpt",desc_file='./data/desc_kitti/00.npy',pose_file="./data/pose_kitti/00.txt"):
    ...
    for v in tqdm(pos_dict.keys()):
        ...
        with torch.no_grad():
            out,_=net(candidates.to(device=device),targets.to(device=device))
            out=out.cpu().numpy()
            ids=np.argsort(-out)
            o=[v]
            o+=ids[:25].tolist()
...
if __name__=='__main__':
    ...
    parser.add_argument('--seq', default='08', help='Sequence to eval. [default: 08]')
    parser.add_argument('--dataset', default="kitti", help="Dataset (kitti or kitti360). [default: kitti]")
    parser.add_argument('--model', default="./model/attention/08.ckpt", help='Model file. [default: "./model/attention/08.ckpt"]')
    parser.add_argument('--desc_file', default='./data/desc_kitti/08.npy', help='File of descriptors. [default: ./data/desc_kitti/08.npy]')
    parser.add_argument('--pairs_file', default='./data/pairs_kitti/neg_100/08.txt', help='Candidate pairs. [default: ./data/pairs_kitti/neg_100/08.txt]')
    parser.add_argument('--pose_file', default="./data/pose_kitti/08.txt", help='Pose file (eval_type=recall). [default: ./data/pose_kitti/08.txt]')
    parser.add_argument('--eval_type', default="f1", help='Type of evaluation (f1 or recall). [default: f1]')
    cfg = parser.parse_args()
    if cfg.dataset=="kitti" and cfg.eval_type=="f1":
        fast_eval(seq=cfg.seq,model_file=cfg.model,desc_file=cfg.desc_file,pair_file=cfg.pairs_file)
