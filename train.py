...
def train(cfg):
    ...
    train_dataset=SigmoidDataset_train(sequs=sequs,neg_ratio=cfg.neg_ratio,eva_ratio=cfg.eval_ratio,desc_folder=cfg.desc_folder,gt_folder=cfg.gt_folder)
    test_dataset=SigmoidDataset_eval(sequs=sequs,neg_ratio=cfg.neg_ratio,eva_ratio=cfg.eval_ratio,desc_folder=cfg.desc_folder,gt_folder=cfg.gt_folder)
    ...
    optimizer=torch.optim.Adam(filter(lambda p: p.requires_grad, net.parameters()),lr=cfg.learning_rate,weight_decay=1e-6)
    epoch=cfg.max_epoch
    starting_epoch=0
    batch_num=0
    if not cfg.model=="":
    ...
    for i in range(starting_epoch,epoch):
        ...
        for i_batch,sample_batch in tqdm(enumerate(train_loader),total=len(train_loader),desc='Train epoch '+str(i),leave=False):
            optimizer.zero_grad()
            out,diff=net(sample_batch["desc1"].to(device=device),sample_batch["desc2"].to(device=device))
            labels=sample_batch["label"].to(device=device)
            loss1=torch.nn.functional.binary_cross_entropy_with_logits(out, labels)
            loss2=labels*diff*diff+(1-labels)*torch.nn.functional.relu(0.2-diff)*torch.nn.functional.relu(0.2-diff)
            ...
            with torch.no_grad():
                ...
                writer.add_scalar('loss1',loss1.cpu().item(),global_step=batch_num)
                writer.add_scalar('loss2',loss2.cpu().item(),global_step=batch_num)
                batch_num+=1
        ...
        torch.save({'epoch': i,'state_dict': net.state_dict(),'optimizer': optimizer.state_dict(),'batch_num': batch_num},os.path.join(cfg.log_dir,cfg.seq,str(i)+'.ckpt'))
...
def test(net,dataloader):
    ...
    with torch.no_grad():
        for i_batch,sample_batch in tqdm(enumerate(dataloader),total=len(dataloader),desc="Eval",leave=False):
            out,_=net(sample_batch["desc1"].to(device=device),sample_batch["desc2"].to(device=device))
            out=out.cpu()
            outlabel=out
...
if __name__=='__main__':
    ...
    parser.add_argument('--seq', default='00', help='Sequence to test. [default: 00]')
    parser.add_argument('--all_seqs', type=list, default=['00','01','02','03','04','05','06','07','08','09','10'], help="All sequence. [default: ['00','01','02','03','04','05','06','07','08','09','10'] ]")
    parser.add_argument('--neg_ratio', type=float, default=1, help='The proportion of negative samples used during training. [default: 1]')
    parser.add_argument('--eval_ratio', type=float, default=0.1, help='Proportion of samples used for validation. [default: 0.1]')
    parser.add_argument('--desc_folder', default="./data/desc_kitti", help='Folder containing descriptors. [default: ./data/desc_kitti]')
    parser.add_argument('--gt_folder', default="./data/gt_kitti", help='Folder containing gt files. [default: ./data/gt_kitti]')
    parser.add_argument('--model', default="", help='Pretrained model. [default: ""]')
    ...
    parser.add_argument('--learning_rate', type=float, default=0.02, help='Initial learning rate. [default: 0.02]')
